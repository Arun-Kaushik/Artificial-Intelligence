...
class Player {
  ...
  public static final Action cDontShoot;
  ...
  double maxProbNextEmission;
    ...
    public Player() {
      ...
      // Initlizing how we signal that we do not wish to shoot a bird.
      cDontShoot = new Action(-1, -1);
  ...
  public int getBirdLargetProb( double [] logProbsBirds , int [] optimalBirdsSpieces ){
    ...
    */
    ...
    // Remove birds that are unknown or black storks.
    logProbsBirds = removeStorksUnknownBirds (optimalBirdsSpieces, logProbsBirds, indexLargestLogVal(logProbsBirds) );

    idxLargestVal = indexLargestLogVal(logProbsBirds);
  ...
  public double [][] getStateProbDistLastTimeStep( double [][] probObsSeq ){

    int [] stateProbLastTimeStep;
    double [][] stateProbLastTimeStepMatrix;

    // Probability distribution fro the last time step is retrived.
    stateProbLastTimeStep = birdHmm.getColumn( probObsSeq, probObsSeq[0].length - 1);
  ...
  public double [][] computeProbabilityNextTimeStep ( int noEmissions, GameState pState, int noBirdObs, int optimalBirdToShoot, int optimalBirdSpecies) {
    ...
    */
    ...
    int [] observationSequence, stateProbLastTimeStep;
    double [][] probNextState, birdAlfaPass, stateProbLastTimeStep;
    HMM birdHmm;
    ...
    stateProbLastTimeStep = getStateProbDistLastTimeStep( probObsSeq );

    // Matrix multiplication between the transition matrix ans the state probability distribution
    // for the last time step.
    probNextState = birdHmm.matrixMultiplication( stateProbLastTimeStep, birdHmm.transitionMatrix );
  ...
  public int mostProbableNextEmission( GameState pState, int noBirdObs, int optimalBirdToShoot, int optimalBirdSpecies ){
    ...
    emissionDistributionNextState = computeProbabilityLastTimeStep ( noEmissions, pState, noBirdObs, optimalBirdToShoot, optimalBirdSpecies )[0];

    // The most probable next emission and its index.
    probNextEmission = maxFinder( emissionDistributionNextState );
    nextemission = getIndexOfLargest( emissionDistributionNextState );
  ...
  public int getBirdsMostProbableSpecies( int [] observstionSequence ){
    ...
    */
    ...
    speciesClassification = indexLargestLogVal( probPerSpieces );
