...
//.nextInt(upperbound-lowerbound) + lowerbound;
        ...
        //int rint=Math.abs(noint.nextInt());
            ...
            for (int i = 0; i < noStates; i++){
                ...
                for (int j = 0; j < noStates; j++){
                    if(i==j){
                    this.transitionMatrix[i][j] = (no.nextDouble()+no.nextDouble()+no.nextDouble()+1)*10;
...
//
// public double [] Viterbi (int [] obsS){
//         int firstobsvalandindex= this.obsS[0];
//         int noStates = this.transitionMatrix[0].length;
//         int noTimeSteps =this.obsS.length;
//
...
//
//         }
//         return getColumn(deltat,noTimeSteps-1);
//
//     }
